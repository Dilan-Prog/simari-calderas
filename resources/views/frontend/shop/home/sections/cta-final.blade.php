...
<section class="svc-cta-final" @if ($previewMode) data-section-id="{{ $section->id }}" @endif>
    ...
    <div class="svc-cta-final__inner">
        @php
            $headlineStyle = $config['headline_style'] ?? null;
            $headlineTag = \App\Support\TextStyle::tag($headlineStyle, 'h2', ['h2', 'h3']);
        @endphp
        @if (!empty($config['headline']))<{{ $headlineTag }}{!! \App\Support\TextStyle::attr($headlineStyle) !!}>{{ $config['headline'] }}</{{ $headlineTag }}>@endif
        @if (!empty($config['subtext']))<p class="svc-cta-final__subtext"{!! \App\Support\TextStyle::attr($config['subtext_style'] ?? null) !!}>{{ $config['subtext'] }}</p>@endif

        <div class="svc-cta-final__actions">
            ...
            @php $secondaryBtn = $config['secondary_button'] ?? null; @endphp
            @if (!empty($secondaryBtn['text']) && !empty($secondaryBtn['url']))
                @php
                    $secStyle = ($secondaryBtn['style'] ?? 'outline') === 'solid' ? 'solid' : 'outline';
                    $secColor = $secondaryBtn['color'] ?? '#ff6213';
                @endphp
                <a href="{{ $secondaryBtn['url'] }}" class="svc-btn svc-btn--{{ $secStyle }}"
                   style="{{ $secStyle === 'solid' ? 'background:'.e($secColor).';border-color:'.e($secColor) : 'color:'.e($secColor).';border-color:'.e($secColor) }}"
                   @if (!str_starts_with($secondaryBtn['url'], '/')) target="_blank" rel="noopener" @endif>
                    {{ $secondaryBtn['text'] }}
                </a>
            @endif
        </div>
