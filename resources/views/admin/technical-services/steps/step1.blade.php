<form id="ts-wizard-form"
      data-step="1"
      data-save-url="{{ route('admin.technical-services.save-step', [$service, 1]) }}"
    ...
    <div class="ts-card">
        ...
        <div class="ts-form-grid" style="gap:1rem">
            ...
            <div class="ts-form-grid ts-form-grid--2">
                <div class="ts-field">
                    ...
                    <input type="text" class="ts-input ts-input--disabled"
                           value="{{ $service->service_number }}" disabled>
                </div>
                <div class="ts-form-grid ts-form-grid--2">
                    <div class="ts-field">
                        <label class="ts-label">Fecha <span class="ts-label__req">*</span></label>
                        <input type="date" name="service_date" class="ts-input" required
                               value="{{ old('service_date', $service->service_date ? \Carbon\Carbon::parse($service->service_date)->format('Y-m-d') : now()->format('Y-m-d')) }}">
                    ...
                    <div class="ts-field">
                        ...
                        <input type="time" name="service_time" class="ts-input" required
                               value="{{ old('service_time', $service->service_time ?? '09:00') }}">
                        <span class="ts-field-error"></span>
                    </div>
                </div>
            </div>
            ...
            <div class="ts-field">
                ...
                <div class="ts-field-select-wrap">
                    <select name="service_type" class="ts-select-field" required>
                        <option value="">Seleccionar tipo...</option>
                        @foreach($serviceTypes as $value => $label)
                            <option value="{{ $value }}"
                                {{ old('service_type', $service->service_type) === $value ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="ts-field">
                    ...
                    <div class="ts-field-select-wrap">
                        <select name="estimated_duration" class="ts-select-field">
                            @foreach(['1 hora','2 horas','3 horas','4 horas','Medio día','Día completo'] as $dur)
                                <option value="{{ $dur }}"
                                    {{ old('estimated_duration', $service->estimated_duration) === $dur ? 'selected' : '' }}>
                                    {{ $dur }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="ts-field">
                    ...
                    <div class="ts-field-select-wrap">
                        <select name="priority" class="ts-select-field">
                            ...
                            <option value="high"   {{ old('priority', $service->priority) === 'high'   ? 'selected' : '' }}>Alta</option>
                            <option value="urgent" {{ old('priority', $service->priority) === 'urgent' ? 'selected' : '' }}>Urgente</option>
                        </select>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                             stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="m6 9 6 6 6-6"/>
