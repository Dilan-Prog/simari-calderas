(function () {
    ...
    var QuoteForm = {
        addProductRow: function (product, isExisting) {
            ...
            tr.dataset.servicePageId = product.service_page_id || '';
        ...
        addServiceRow: function (s) {
            // Called by inline service search — mismo layout que addRow(), pero
            // sin SKU/stock, y marca la fila con servicePageId en vez de productId.
            var existingEmpty = document.getElementById('emptyRow');
            if (existingEmpty) existingEmpty.remove();
            ...
            tr.dataset.servicePageId = s.id || '';
            tr.dataset.productSku    = '';
            tr.innerHTML =
                '<td class="col-num">' + rowCount + '</td>' +
                '<td class="col-name">' +
                    '<div class="item-name-wrap">' +
                        '<div class="item-name-row">' +
                            '<input type="text" class="cell-input item-name" value="' + _esc(s.name || '') + '" placeholder="Nombre del producto o servicio">' +
                            '<button type="button" class="item-desc-toggle" title="Agregar descripción detallada" aria-label="Agregar descripción">' +
                                '<svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>' +
                            '</button>' +
                        '</div>' +
                ...
                '</td>' +
                '<td class="col-sku">' +
                    '<input type="text" class="cell-input item-sku sku-readonly" value="" readonly disabled tabindex="-1" placeholder="Servicio">' +
                '</td>' +
                '<td class="col-qty">' +
                    '<input type="number" class="cell-input item-qty" value="1" min="1" step="1">' +
                '</td>' +
        ...
        serializeItems: function () {
            ...
            rows.forEach(function (tr) {
                ...
                items.push({
                    ...
                    service_page_id:  tr.dataset.servicePageId || null,
